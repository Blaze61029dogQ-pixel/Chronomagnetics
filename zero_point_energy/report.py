...
def main() -> None:
    ...
    _check("Proca / scalar ratio (D32)", proca_rho / scalar_rho, 3.0, tolerance=1e-12)
    ...
    _check("Massive-field exact vs. asymptotic (D3/D4)", asymptotic, exact, tolerance=1e-9)
    ...
    _check("V_zpf == Q_zpf/C (D59)", v_zpf, q_zpf / capacitance, tolerance=1e-12)
    _check("V_zpf == omega0*Phi_zpf (D59)", v_zpf, omega0 * phi_zpf, tolerance=1e-12)
    _check("I_zpf == Phi_zpf/L (D59)", i_zpf, phi_zpf / inductance, tolerance=1e-12)
    _check("I_zpf == omega0*Q_zpf (D59)", i_zpf, omega0 * q_zpf, tolerance=1e-12)
    ...
    _check("Ring/interval ratio (D25 vs D26)", ring / interval, 4.0, tolerance=1e-12)

    # D29: T_zz of the plate stress tensor must equal the plate pressure.
    gap = 250e-9
    t00, txx, tyy, tzz = casimir.stress_tensor_diagonal(gap)
    _check("Stress tensor T_zz == P (D29)", tzz, casimir.plate_pressure(gap), tolerance=1e-12)

    # D6/D10: vacuum equation of state p = -rho, independent of rho's value.
    rho_vac = 4.2e30
    _check("Vacuum EOS p = -rho (D6/D10)", stress_tensor.vacuum_pressure(rho_vac), -rho_vac, tolerance=1e-12)
    ...
    _check("Hawking T(1 Msun) order of magnitude", t_h, 6.17e-8, tolerance=1e-2)
    years = thermal.evaporation_time_benchmark(solar_mass) / 3.15576e7
    _check("Evaporation time(1 Msun) [yr] order of magnitude", years, 2.1e67, tolerance=1e-2)
    ...
    _check("Schwinger critical field, electron", e_s, 1.323e18, tolerance=1e-3)
